...
export type PositionedDraggableProps = {
...
function getCurrentPosition(draggable) {
  const node = (ReactDOM.findDOMNode(draggable): any);
  const rect = (node: Element).getBoundingClientRect();
  const offsetParent = draggable.props.offsetParent || node.offsetParent || node.ownerDocument.body;
  return offsetXYFromParent({clientX: rect.left, clientY: rect.top}, offsetParent, 1);
}
...
export default class PositionedDraggable extends React.Component<PositionedDraggableProps, DraggableState> {

  static displayName = 'PositionedDraggable';
  ...
  // React 16.3+
  // Arity (props, state)
  static getDerivedStateFromProps({position}: PositionedDraggableProps, {prevPropsPosition}: DraggableState) {
  ...
  constructor(props: PositionedDraggableProps) {
    super(props);
